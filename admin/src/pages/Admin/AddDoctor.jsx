...
const AddDoctor = () => {
  ...
  const onsubmitHandler = async(event)=>{
    ...
    try {
      ...
      const {data} = await axios.post(backendUrl + '/api/admin/add-doctor', formData, {headers:{aToken, 'Content-Type':'multipart/form-data'}})
      if (data.success) {
  ...
  return (
    <form onSubmit={onsubmitHandler} className='m-5 w-full'>
      ...
      <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
        ...
        <button type='submit' className='bg-gradient-to-r from-[#0ea5e9] to-[#22c55e] text-white px-10 py-3 mt-4 rounded-full cursor-pointer'>
          Add doctor
        </button>
